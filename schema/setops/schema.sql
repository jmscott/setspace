...
COMMENT ON TABLE flowd_schema IS
  'Schemas processed by flowd process in schema/<schema>'
;
...
(
	...
	condition_name	text CHECK (
				condition_name ~ '^[[:alpha:]_]{1,63}$'
			) NOT NULL
);
COMMENT ON TABLE pg_sql_error IS
  'Map PostgreSQL error codes to condition names' 
;

INSERT INTO pg_sql_error VALUES
