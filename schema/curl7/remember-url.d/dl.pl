#
#  Synopsis:
#	Build an html <dl> results set of remembered url.
#
...
if ($host) {
	$sql_from_host =<<END;
	  inner join remember_url_host h on (h.blob = r.blob)
END
	$host =~ s/\\/\\\\/g;
	$host =~ s/'/\\'/g;
	$sql_qual_host =<<END;
...
select
	...
	coalesce(
		...
		rut.value,		/* remember_url_title */
		s.url			/* blob title */
	) as "title"
    from
	search s
	  left outer join remember_url_title rut on (s.blob = rut.blob)
...
<dt>
 <a
   title="$title"
   target="_blank"
   href="/cgi-bin/remember-url?out=click&amp;udig=$blob">$title</a>
</dt>
<dd>
 <div class="meta">
