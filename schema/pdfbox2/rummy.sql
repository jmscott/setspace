...
UNION (
  SELECT
    ...
    FROM
	  ...
	  LEFT OUTER JOIN pdfbox2.extract_pages_utf8_pending exp ON (
	  	exp.blob = pd.blob
	  )
	  LEFT OUTER JOIN pdfbox2.merge_pages_text_utf8 txt ON (
	  	txt.blob = pd.blob
	  )
	  LEFT OUTER JOIN pdfbox2.merge_pages_text_utf8_pending txtp ON (
	  	txtp.blob = pd.blob
	  )
	  LEFT OUTER JOIN pdfbox2.merge_pages_tsv_utf8 tsv ON (
	  	tsv.blob = pd.blob
	  )
	  LEFT OUTER JOIN pdfbox2.merge_pages_tsv_utf8_pending tsvp ON (
	  	tsvp.blob = pd.blob
	  )
    WHERE
    	pd.exit_status = 0
