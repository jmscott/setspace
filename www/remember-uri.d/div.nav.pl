#
#  Synopsis:
#	HTML Navigation bar for remember_uri search results
#
require 'dbi.pl';
...
if ($host) {
	...
	$host =~ s/'/\\'/g;
	$sql_from_host =<<END;
inner join remember_uri_host h on (h.blob = u.blob)
END
	$sql_qual_host =<<END;
and
h.value = E'$host' 
...
my $Q = dbi_select(
	db	=>	dbi_connect(),
	tag	=>	'remember-uri-div.nav-count',
	#dump	=>	'>/tmp/remember-uri-div.nav-count.sql',
	sql	=>	<<END
select
